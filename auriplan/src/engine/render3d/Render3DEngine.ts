...
class Render3DEngine {
  ...
  private setupGround(): void {
    this.gridHelper = new THREE.GridHelper(50, 20, 0x888888, 0xcccccc);
    this.gridHelper.position.y = 0;
    this.scene.add(this.gridHelper);

    const planeGeometry = new THREE.PlaneGeometry(50, 50);
    const planeMaterial = new THREE.MeshStandardMaterial({ color: 0xf0f0f0, transparent: true, opacity: 0.15, side: THREE.DoubleSide });
  ...
  syncScene(data: {
    ...
    this.scene.traverse(obj => {
      if (obj.userData?.type && obj.userData.type !== 'ground') {
        toRemove.push(obj);
      }
    });
  ...
  private createWallMesh(wall: any): void {
    ...
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    mesh.userData = { type: 'wall', id: wall.id, originalData: wall };
    this.addObject(mesh);
  }
  ...
  private fitCameraToScene(): void {
    ...
    this.scene.traverse(obj => {
      if (obj instanceof THREE.Mesh && obj.userData?.type && obj.userData.type !== 'ground') {
